...
export default function Delete() {
    ...
    const { loading, result } = useQueryEffect(() => api.ProjectManage_GetProjectById(projectid));

    let validConfirmCode = '';

    if (!loading) {
        validConfirmCode = (result.robiniaUrlPrefix + '_' + result.id).toUpperCase();
    }

    const dlc = [
        { title: 'Id', key: 'id' },
        { title: 'Robinia URL prefix', key: 'robiniaUrlPrefix' },
